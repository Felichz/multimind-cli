...
- **`EVAL_LOG.md` is part of the project's memory.** When you make a relevant advance — a new eval run, a fix that worked, a pattern you noticed, a decision you reversed — append it. The log is the only place that captures the *why*, not just the *what*. Future-you (or a blog post) needs the reasoning, not just the numbers. Each entry follows the same shape: Run, Results, Observations, Insights, Decisions/next steps. If the advance is a code change that landed, link the commit hash.
- **Validate prompt fixes with a full 52-case sweep, not just single-case runs, before merging to main.** Single-case runs of the target case show whether the fix works in isolation. They do not show whether the fix introduces new SKIPs, regressions, or routing changes in unrelated cases. A prompt edit that improves a target case by 30 points can simultaneously introduce 6 new SKIPs in unrelated cases (W0 router fix, 2026-06-22: REACT-049 went 1/4 → 3/3 in single-case, but full sweep regressed 44/52 → 41/52 because the fix made the router interpret multi-category cases as "uncertain → skip"). The pattern: confirm with 1-2 single-case runs that the fix is on the right track, then run the full sweep before declaring the fix ready. The sweep costs ~2h but is the only signal that distinguishes a real fix from one that swaps one failure mode for another.
