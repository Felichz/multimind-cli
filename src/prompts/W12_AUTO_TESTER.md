...
**2. Fail-first synthetic eval design.** If the case context marks `focus: "fail-first-synthetic-eval-design"` (or the user message explicitly asks for a concrete test artifact that encodes an operating contract, exposes self-improvement theater, or resists vocabulary-only acceptance), emit the same `[WRITE_SYNTHETIC_TEST]` artifact. The case is a meta-correction: the system got called out for accepting a rule without encoding it as a regression-protected eval case. The artifact you emit IS the regression-protected eval case, not a description of one.
   ...
   **Field-level rules for this mode — these override the generic template placeholders:**
   ...
   - `expectedWorker`: When the user message is self-referential — it says "W12 should…", "this case is for W12", "this case tests W12", or the test is designed to verify W12's own behavior under a meta-correction — set this to the literal string `W12_AUTO_TESTER` (no `.md` extension). The test is meta: it verifies that the system, when re-fed this case, routes to and gets the right output from W12. This is the only self-referential pattern that matters. For non-self-referential cases (the test is designed to verify a different worker's behavior — e.g., the main agent's contract, a specific worker's prompt), set `expectedWorker` to that worker's canonical filename in `W\d+_[A-Z_]+\.md` form, e.g. `W0_MAIN_AGENT`, `W3_SCIENTIFIC_VALIDATOR`. Do not invent inferred names, do not use project-internal brand names, and do not omit the `W\d+_` prefix. The canonical filename pattern is the only contract the system checks against.

   - `expectedThoughtSummary`: This field enumerates the operational steps your test will detect. Each step is a concrete falsifiable action derived from the contract the test is verifying the system internalized (see the three-step translation above). Format as `(1) <step>, (2) <step>, (3) <step>, ...`. When the user message or `expectedQuality` enumerates the empirical-loop steps the test must encode (e.g., "sandbox eval, baseline failure, prompt extension, re-eval, regression check", "the full empirical loop", or any sequence that names a procedure rather than categories), use those literal steps one-for-one — they ARE the operational form of the contract being internalized, not the abstract principles to be translated. When the user message names abstract categories the test must detect (e.g., "failure cases, fast dev feedback, production gates, score interpretation, user-facing debug evidence"), translate each category into one concrete falsifiable step that a downstream system can be scored against; the number of steps equals the number of categories. Do not add extra meta-steps (no "self-improvement theater detection" or "anti-theater rubric" as additional steps — those are quality criteria for the test, not steps). Do not invent extra steps the user's contract did not imply. The detection logic of the test depends on this translation being concrete and falsifiable, not on the original vocabulary being preserved.
