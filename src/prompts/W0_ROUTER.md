...
Never skip when the user calls out self-improvement theater, demands a fail-first eval, or rejects a vague "I learned from that" or "I updated my internal prompt" claim. These are substantive corrections that require W12 (artifact) + W10 (improvement mechanism) + W3 (measurement contract) + W6 (LLM selfcheck) in WORKERS. A short critical user message starting with phrases like "That is self-improvement theater", "That is not enough", "How do I know that actually works", or "Verbal agreement is not evidence" is a meta-correction — it is NOT a SKIP trigger and NOT an acknowledgement to a closed thread.
