...
export function consolidateSynthesizerForMainAgent(input: { synthesis: string; insights: ConsolidatorInsight[] }) {
...
function parseSubconsciousDecision(text: string) {
  ...
  if (finalLine === "[subconscious:safe_to_end]" || finalLine === "subconscious:safe_to_end") return "safe_to_end"
...
function hasMetricReadinessSignal(insights: ConsolidatorInsight[]) {
  ...
  return (
    ...
    hasAnySignal(output, ["100%", "precision", "score", "benchmark", "metric", "dataset", "coverage", "puntaje"])
  )
}
...
function hasAutonomousAgentDevelopmentSignal(insights: ConsolidatorInsight[]) {
  const output = insights.map((insight) => insight.output).join("\n")
  return (
    hasAnySignal(output, ["autonomous agent", "review-agent", "review agent", "triage agent", "agent prompt"]) &&
    hasAnySignal(output, [
      "baseline",
      "fail-first",
      "same-case",
